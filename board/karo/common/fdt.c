...
static char *karo_fdt_overlay_filename(const char *prefix, const char *overlay,
				       const char *dev_type, const char *dev_part)
{
	...
	malloc_size = strlen(prefix) + strlen(overlay) + strlen(ext) + 2;
	fdtfile = malloc(malloc_size);
	if (!fdtfile)
		return ERR_PTR(-ENOMEM);

	snprintf(fdtfile, malloc_size, "%s-%s%s", prefix, overlay, ext);
	if (file_exists(dev_type, dev_part, fdtfile, FS_TYPE_ANY))
		return fdtfile;
	/* if the .dtbo file is not found, try with extension .dtb for compatibility */
	fdtfile[strlen(fdtfile) - 1] = '\0';
	if (file_exists(dev_type, dev_part, fdtfile, FS_TYPE_ANY)) {
		printf("%so does not exist; fall back to %s\n", fdtfile, fdtfile);
		if (first) {
			printf("---> Please update your bootfs!\n");
			first = false;
		}
		return fdtfile;
	}
...
#ifdef DEBUG
static bool fdt_overlay_debug = 1;
#else
static bool fdt_overlay_debug;
#endif
			  ...
			  const char *overlay)
{
	...
	char *filename = karo_fdt_overlay_filename(soc_family, overlay, dev_type, dev_part);

	if (IS_ERR(filename))
		return PTR_ERR(filename);

	if (!filename) {
		filename = karo_fdt_overlay_filename(soc_prefix, overlay, dev_type, dev_part);
		if (IS_ERR(filename))
			return PTR_ERR(filename);
		if (!filename) {
			printf("overlay file '(%s|%s)-%s.dtb[o]' does not exist\n",
			       soc_prefix, soc_family, overlay);
			return -ENOENT;
		}
	}
...
int karo_load_fdt(const char *fdt_file)
{
	...
	int mmcpart = 1;
	char dev_part[8];
	ulong fdt_addr = env_get_hex("fdt_addr", CFG_FDTADDR);
	loff_t size;
	void *fdt;
...
#if IS_ENABLED(CONFIG_FDT_FIXUP_PARTITIONS) && !IS_ENABLED(CONFIG_XPL_BUILD)
