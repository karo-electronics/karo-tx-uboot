...
int power_init_board(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(ver); i++) {
		ret = pmic_reg_read(pmic_dev, RN5T567_LSIVER + i);
		if (ret < 0) {
	...
	for (i = 0; i < ARRAY_SIZE(pmic_vals); i++) {
		...
		ret = pmic_update_reg(pmic_dev, p->addr, p->val, p->mask, p->name);
		if (ret)
			break;
	}
	if (ret)
		printf("PMIC:  %s LSI ver: 0x%02x OTP ver: 0x%02x FAILED: %d\n",
		       PMIC_NAME, ver[0], ver[1], ret);
	else if (CONFIG_IS_ENABLED(BANNER_PRINT))
		printf("PMIC:  %s LSI ver: 0x%02x OTP ver: 0x%02x OK\n",
		       PMIC_NAME, ver[0], ver[1]);
