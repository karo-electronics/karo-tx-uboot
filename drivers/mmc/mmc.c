...
int mmc_set_psa(struct mmc *mmc, u8 mode, lbaint_t blkcnt)
{
	...
	mmc->gen_cmd6_time = (1 << ext_csd[EXT_CSD_PSA_TIMEOUT]) / 10;
	debug("psa_timeout=%08x: %u\n", ext_csd[EXT_CSD_PSA_TIMEOUT],
	      mmc->gen_cmd6_time);

	if (value >= 0 && value != (ext_csd[EXT_CSD_PSA_ENABLEMENT] & 0x30)) {
...
out:
	debug("Restoring gen_cmd6_time to %u\n", gen_cmd6_time);
