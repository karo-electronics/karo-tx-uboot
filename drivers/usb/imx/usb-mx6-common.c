...
#define UCTRL_PWR_POL			BIT(9) /* OTG Polarity of Power Pin */
...
#define PLL_USB_EN_USB_CLKS_MASK	BIT(6)
#define PLL_USB_PWR_MASK		BIT(12)
#define PLL_USB_ENABLE_MASK		BIT(13)
#define PLL_USB_BYPASS_MASK		BIT(16)
#define PLL_USB_REG_ENABLE_MASK		BIT(21)
#define PLL_USB_DIV_SEL_MASK		(0x07 << 22)
#define PLL_USB_LOCK_MASK		BIT(31)
...
int usb_phy_mode(int port)
{
	struct usbnc_regs *usbnc = (struct usbnc_regs *)(uintptr_t)USB_BASE_ADDR + USBNC_OFFSET +
		0x10000 * port;
	void __iomem *status = &usbnc->phy_status;
	u32 val;
...
{
	struct anatop_regs __iomem *anatop = anatop_base;
...
#else
static inline void usb_power_config_mx6(void __iomem *anatop_base, int anatop_bits_index)
{
}
#endif
...
{
	struct usbnc_regs __iomem *usbnc = usbnc_base;
...
#else
static inline void usb_power_config_mx7(void __iomem *usbnc_base)
{
}
#endif
...
#else
static inline void usb_power_config_mx7ulp(void __iomem *usbphy_base)
{
}
#endif
...
#else
static inline void usb_power_config_imx8(void __iomem *usbphy_base)
{
}
#endif
