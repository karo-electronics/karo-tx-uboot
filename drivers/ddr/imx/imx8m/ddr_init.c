			   ...
			   unsigned int range_address)
{
	...
	ret = read_poll_timeout(reg32_read, tmp, (tmp & 3) == 2, 1, 10000, DDRC_SBRSTAT(0));
	if (ret)
		return ret;
...
int ddr_init(struct dram_timing_info *dram_timing)
{
	...
	reg32_write(DDRC_SWCTL(0), 0x00000001);
	ret = read_poll_timeout(reg32_read, tmp, tmp & 1, 1, 10000, DDRC_SWSTAT(0));
	if (ret)
		return ret;
	/*
	 ...
	 */
	ret = read_poll_timeout(reg32_read, tmp, !(tmp & 1), 1, 10000, DDRPHY_CalBusy(0));
	if (ret)
		return ret;
	...
	ret = read_poll_timeout(reg32_read, tmp, tmp & 1, 1, 10000, DDRC_DFISTAT(0));
	if (ret)
		return ret;
	...
	ret = read_poll_timeout(reg32_read, tmp, (tmp & 3) == 1, 1, 10000, DDRC_STAT(0));
	if (ret)
		return ret;
