...
static void fec_reset(struct eth_device *dev);
...
struct fec_info_s fec_info[] = {
	{
		0,			/* index */
		(void *)CONFIG_FEC0_IOBASE,	/* io base */
...
static inline int __fec_mii_info(volatile fec_t *fecp, unsigned char addr)
{
	...
	printf("PHY identify @ 0x%x = 0x%08x\n", addr, id);
	return 0;
}
...
static int mxc_fec_mii_discover_phy(struct eth_device *dev)
{
	...
	volatile fec_t *fecp = info->iobase;
...
#ifdef CONFIG_ARCH_MMU
	memcpy(ioremap_nocache(info->txbd[info->txIdx].cbd_bufaddr,
			length), (void *)packet, length);
#else
	info->txbd[info->txIdx].cbd_bufaddr = (ulong)packet;
#endif
...
#ifndef ET_DEBUG
static inline void dbgFecRegs(struct eth_device *dev)
{
}
#else
static void dbgFecRegs(struct eth_device *dev)
{
	struct fec_info_s *info = dev->priv;
	volatile fec_t *fecp = info->iobase;
	...
	printf("ievent       %p - %08x\n", &fecp->eir, fecp->eir);
	printf("imask        %p - %08x\n", &fecp->eimr, fecp->eimr);
	printf("r_des_active %p - %08x\n", &fecp->rdar, fecp->rdar);
	printf("x_des_active %p - %08x\n", &fecp->tdar, fecp->tdar);
	printf("ecntrl       %p - %08x\n", &fecp->ecr, fecp->ecr);
	printf("mii_mframe   %p - %08x\n", &fecp->mmfr, fecp->mmfr);
	printf("mii_speed    %p - %08x\n", &fecp->mscr, fecp->mscr);
	printf("mii_ctrlstat %p - %08x\n", &fecp->mibc, fecp->mibc);
	printf("r_cntrl      %p - %08x\n", &fecp->rcr, fecp->rcr);
	printf("x_cntrl      %p - %08x\n", &fecp->tcr, fecp->tcr);
	printf("padr_l       %p - %08x\n", &fecp->palr, fecp->palr);
	printf("padr_u       %p - %08x\n", &fecp->paur, fecp->paur);
	printf("op_pause     %p - %08x\n", &fecp->opd, fecp->opd);
	printf("iadr_u       %p - %08x\n", &fecp->iaur, fecp->iaur);
	printf("iadr_l       %p - %08x\n", &fecp->ialr, fecp->ialr);
	printf("gadr_u       %p - %08x\n", &fecp->gaur, fecp->gaur);
	printf("gadr_l       %p - %08x\n", &fecp->galr, fecp->galr);
	printf("x_wmrk       %p - %08x\n", &fecp->tfwr, fecp->tfwr);
	printf("r_bound      %p - %08x\n", &fecp->frbr, fecp->frbr);
	printf("r_fstart     %p - %08x\n", &fecp->frsr, fecp->frsr);
	printf("r_drng       %p - %08x\n", &fecp->erdsr, fecp->erdsr);
	printf("x_drng       %p - %08x\n", &fecp->etdsr, fecp->etdsr);
	printf("r_bufsz      %p - %08x\n", &fecp->emrbr, fecp->emrbr);
#if 0
	printf("\n");
	printf("rmon_t_drop        %p - %08x\n", &fecp->rmon_t_drop,
	       fecp->rmon_t_drop);
	printf("rmon_t_packets     %p - %08x\n", &fecp->rmon_t_packets,
	       fecp->rmon_t_packets);
	printf("rmon_t_bc_pkt      %p - %08x\n", &fecp->rmon_t_bc_pkt,
	       fecp->rmon_t_bc_pkt);
	printf("rmon_t_mc_pkt      %p - %08x\n", &fecp->rmon_t_mc_pkt,
	       fecp->rmon_t_mc_pkt);
	printf("rmon_t_crc_align   %p - %08x\n", &fecp->rmon_t_crc_align,
	       fecp->rmon_t_crc_align);
	printf("rmon_t_undersize   %p - %08x\n", &fecp->rmon_t_undersize,
	       fecp->rmon_t_undersize);
	printf("rmon_t_oversize    %p - %08x\n", &fecp->rmon_t_oversize,
	       fecp->rmon_t_oversize);
	printf("rmon_t_frag        %p - %08x\n", &fecp->rmon_t_frag,
	       fecp->rmon_t_frag);
	printf("rmon_t_jab         %p - %08x\n", &fecp->rmon_t_jab,
	       fecp->rmon_t_jab);
	printf("rmon_t_col         %p - %08x\n", &fecp->rmon_t_col,
	       fecp->rmon_t_col);
	printf("rmon_t_p64         %p - %08x\n", &fecp->rmon_t_p64,
	       fecp->rmon_t_p64);
	printf("rmon_t_p65to127    %p - %08x\n", &fecp->rmon_t_p65to127,
	       fecp->rmon_t_p65to127);
	printf("rmon_t_p128to255   %p - %08x\n", &fecp->rmon_t_p128to255,
	       fecp->rmon_t_p128to255);
	printf("rmon_t_p256to511   %p - %08x\n", &fecp->rmon_t_p256to511,
	       fecp->rmon_t_p256to511);
	printf("rmon_t_p512to1023  %p - %08x\n", &fecp->rmon_t_p512to1023,
	       fecp->rmon_t_p512to1023);
	printf("rmon_t_p1024to2047 %p - %08x\n", &fecp->rmon_t_p1024to2047,
	       fecp->rmon_t_p1024to2047);
	printf("rmon_t_p_gte2048   %p - %08x\n", &fecp->rmon_t_p_gte2048,
	       fecp->rmon_t_p_gte2048);
	printf("rmon_t_octets      %p - %08x\n", &fecp->rmon_t_octets,
	       fecp->rmon_t_octets);

	printf("\n");
	printf("ieee_t_drop      %p - %08x\n", &fecp->ieee_t_drop,
	       fecp->ieee_t_drop);
	printf("ieee_t_frame_ok  %p - %08x\n", &fecp->ieee_t_frame_ok,
	       fecp->ieee_t_frame_ok);
	printf("ieee_t_1col      %p - %08x\n", &fecp->ieee_t_1col,
	       fecp->ieee_t_1col);
	printf("ieee_t_mcol      %p - %08x\n", &fecp->ieee_t_mcol,
	       fecp->ieee_t_mcol);
	printf("ieee_t_def       %p - %08x\n", &fecp->ieee_t_def,
	       fecp->ieee_t_def);
	printf("ieee_t_lcol      %p - %08x\n", &fecp->ieee_t_lcol,
	       fecp->ieee_t_lcol);
	printf("ieee_t_excol     %p - %08x\n", &fecp->ieee_t_excol,
	       fecp->ieee_t_excol);
	printf("ieee_t_macerr    %p - %08x\n", &fecp->ieee_t_macerr,
	       fecp->ieee_t_macerr);
	printf("ieee_t_cserr     %p - %08x\n", &fecp->ieee_t_cserr,
	       fecp->ieee_t_cserr);
	printf("ieee_t_sqe       %p - %08x\n", &fecp->ieee_t_sqe,
	       fecp->ieee_t_sqe);
	printf("ieee_t_fdxfc     %p - %08x\n", &fecp->ieee_t_fdxfc,
	       fecp->ieee_t_fdxfc);
	printf("ieee_t_octets_ok %p - %08x\n", &fecp->ieee_t_octets_ok,
	       fecp->ieee_t_octets_ok);
	printf("\n");
	printf("rmon_r_drop        %p - %08x\n", &fecp->rmon_r_drop,
	       fecp->rmon_r_drop);
	printf("rmon_r_packets     %p - %08x\n", &fecp->rmon_r_packets,
	       fecp->rmon_r_packets);
	printf("rmon_r_bc_pkt      %p - %08x\n", &fecp->rmon_r_bc_pkt,
	       fecp->rmon_r_bc_pkt);
	printf("rmon_r_mc_pkt      %p - %08x\n", &fecp->rmon_r_mc_pkt,
	       fecp->rmon_r_mc_pkt);
	printf("rmon_r_crc_align   %p - %08x\n", &fecp->rmon_r_crc_align,
	       fecp->rmon_r_crc_align);
	printf("rmon_r_undersize   %p - %08x\n", &fecp->rmon_r_undersize,
	       fecp->rmon_r_undersize);
	printf("rmon_r_oversize    %p - %08x\n", &fecp->rmon_r_oversize,
	       fecp->rmon_r_oversize);
	printf("rmon_r_frag        %p - %08x\n", &fecp->rmon_r_frag,
	       fecp->rmon_r_frag);
	printf("rmon_r_jab         %p - %08x\n", &fecp->rmon_r_jab,
	       fecp->rmon_r_jab);
	printf("rmon_r_p64         %p - %08x\n", &fecp->rmon_r_p64,
	       fecp->rmon_r_p64);
	printf("rmon_r_p65to127    %p - %08x\n", &fecp->rmon_r_p65to127,
	       fecp->rmon_r_p65to127);
	printf("rmon_r_p128to255   %p - %08x\n", &fecp->rmon_r_p128to255,
	       fecp->rmon_r_p128to255);
	printf("rmon_r_p256to511   %p - %08x\n", &fecp->rmon_r_p256to511,
	       fecp->rmon_r_p256to511);
	printf("rmon_r_p512to1023  %p - %08x\n", &fecp->rmon_r_p512to1023,
	       fecp->rmon_r_p512to1023);
	printf("rmon_r_p1024to2047 %p - %08x\n", &fecp->rmon_r_p1024to2047,
	       fecp->rmon_r_p1024to2047);
	printf("rmon_r_p_gte2048   %p - %08x\n", &fecp->rmon_r_p_gte2048,
	       fecp->rmon_r_p_gte2048);
	printf("rmon_r_octets      %p - %08x\n", &fecp->rmon_r_octets,
	       fecp->rmon_r_octets);

	printf("\n");
	printf("ieee_r_drop      %p - %08x\n", &fecp->ieee_r_drop,
	       fecp->ieee_r_drop);
	printf("ieee_r_frame_ok  %p - %08x\n", &fecp->ieee_r_frame_ok,
	       fecp->ieee_r_frame_ok);
	printf("ieee_r_crc       %p - %08x\n", &fecp->ieee_r_crc,
	       fecp->ieee_r_crc);
	printf("ieee_r_align     %p - %08x\n", &fecp->ieee_r_align,
	       fecp->ieee_r_align);
	printf("ieee_r_macerr    %p - %08x\n", &fecp->ieee_r_macerr,
	       fecp->ieee_r_macerr);
	printf("ieee_r_fdxfc     %p - %08x\n", &fecp->ieee_r_fdxfc,
	       fecp->ieee_r_fdxfc);
	printf("ieee_r_octets_ok %p - %08x\n", &fecp->ieee_r_octets_ok,
	       fecp->ieee_r_octets_ok);
#endif
	printf("=====\n");
}
...
static int fec_init(struct eth_device *dev, bd_t *bd)
{
...
#endif
#endif	/* CONFIG_CMD_MII || CONFIG_MII || CONFIG_SYS_DISCOVER_PHY */
	...
	for (i = 0; i < TX_BUF_CNT; i++) {
		...
		info->txbd[i].cbd_bufaddr =
			iomem_to_phys((ulong)&info->txbuf[0]);
#else
		info->txbd[i].cbd_bufaddr = (ulong)&info->txbuf[0];
#endif
	}
...
static void fec_halt(struct eth_device *dev)
{
	...
	/* Do NOT reset the chip as this will invalidate the MAC address */
	fecp->ecr &= ~FEC_ECR_ETHER_EN;
#else
	fec_reset(dev);
#endif
...
int mxc_fec_initialize(bd_t *bis)
{
	...
	for (i = 0; i < sizeof(fec_info) / sizeof(fec_info[0]); i++) {
...
#ifdef CONFIG_ARCH_MMU
		fec_info[i].rxbd =
			ioremap_nocache(iomem_to_phys((ulong)memalign(CONFIG_SYS_CACHELINE_SIZE,
		...
		for (j = 0; j < PKTBUFSRX; ++j) {
			fec_info[i].rxbuf[j] =
				ioremap_nocache(iomem_to_phys((ulong)memalign(PKTSIZE_ALIGN,
						PKTSIZE)),
						PKTSIZE_ALIGN);
		}
#else
		...
		fec_info[i].txbd =
		    memalign(CONFIG_SYS_CACHELINE_SIZE,
				       (TX_BUF_CNT * sizeof(cbd_t)));
		fec_info[i].txbuf =
		    memalign(CONFIG_SYS_CACHELINE_SIZE, DBUF_LENGTH);
#endif

#ifdef ET_DEBUG
		printf("%s: rxbd %p txbd %p ->%p\n", dev->name,
		       fec_info[i].rxbd, fec_info[i].txbd,
		       fec_info[i].txbuf);
#endif

		fec_info[i].phy_name =
			memalign(CONFIG_SYS_CACHELINE_SIZE, 32);
